...
function log(req: Request, status: number, durationMs: number) {
  ...
  console.log(
    `[${ts}] ${req.method} ${new URL(req.url).pathname} → ${status} (${durationMs}ms)`
  );
}

// ── Factory ────────────────────────────────────────────────────────────────

export function createHandler(databaseUrl: string) {
  ...
  async function handle(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (
        ...
        if (!serviceId) {
          res = notFound(`Service '${segments[1]}' not found`);
        } else {
          ...
          if (depthParam !== null) {
            ...
            if (isNaN(depth) || depth < 1 || depth > 10) {
              ...
              const deps = await getTransitiveDependencies(serviceId, depth, databaseUrl);
              res = json({ serviceId, depth, dependencies: deps });
            }
          } else {
            const deps = await getDirectDependencies(serviceId, databaseUrl);
        ...
        if (!serviceId) {
          ...
          const owner = await getServiceOwner(serviceId, databaseUrl);
        ...
        if (!serviceId) {
          ...
          const impacted = await findImpactRadius(serviceId, databaseUrl);
        ...
        if (!serviceId) {
          ...
          if (isNaN(hours) || hours < 1 || hours > 720) {
            ...
            const deploys = await getRecentDeployments(serviceId, hours, databaseUrl);
  ...
  return { handle, close: () => client.end() };
}
